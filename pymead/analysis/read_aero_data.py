...
def read_forces_from_mses(search_file: str):
    """
    This method uses regex to extract the aerodynamic force coefficients of the airfoil [system] from the "list forces"
    option available in the MPLOT menu
    :param search_file: "Forces" file to be read (log of the MPLOT "list forces" menu selected output)
    :return: Dictionary containing information about the aerodynamic force coefficients ({'CL': <CL value>,
...
def read_bl_data_from_mses(src_file: str):
    ...
    :param src_file: Boundary layer file (bl.<airfoil name>)
    :return: Boundary layer data list (length m) of dictionaries, where m is the number of airfoil sides (2 times the
    number of airfoils)
    """
    with open(src_file, 'r') as f:
        line1 = f.readline()
