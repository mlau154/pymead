...
from pymead.utils.transformations import rotate, translate
...
class AirfoilParametrization:
    ...
    def __init__(self, param_setup: ParamSetup, generate_airfoils: callable, args=None):
        ...
        `AirfoilParametrization` is `pymead`'s way of linking the shape, position, or angle of one or more `Airfoil`s
        to an external set of parameters. For example, an `Airfoil` could be placed a certain distance from another
        `Airfoil` by the value contained in a `Param`. See `pymead.examples` for example implementations.
    ...
    def mirror(self, axis: np.ndarray or tuple, fixed_airfoil_idx: int, linked_airfoil_idx: int,
        ...
        Sets a portion of a linked `pymead.core.airfoil.Airfoil` as the mirror across a specified axis of a portion
        of a fixed `pymead.core.airfoil.Airfoil`.
        ...
        `fixed_anchor_point_range`: a 2-item `tuple` representing the range of `pymead.core.anchor_point.AnchorPoint`s
        to be mirrored. The first item should be a `str` representing the name of the first
        `pymead.core.anchor_point.AnchorPoint` in the fixed airfoil, and the second item should be a `str`
        representing the name of the final `pymead.core.anchor_point.AnchorPoint` in the fixed airfoil
    ...
    def clone(self):
        ...
        Clones an `pymead.core.airfoil.Airfoil`
        """
        pass
