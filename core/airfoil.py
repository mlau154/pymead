...
############################# FINISH LATER #################################
        ...
        for idx, obj in enumerate([var for var in vars(self.base_airfoil_params).values()
                                   if isinstance(var, Param) and var.active and not var.linked]):
            obj.value = parameters[idx]
            param_idx += 1

        for anchor_point in self.anchor_point_tuple:

            for idx, obj in enumerate([var.value for var in vars(anchor_point).values()
                                if isinstance(var, Param) and var.active and not var.linked]):
                obj.value = parameters[idx]
